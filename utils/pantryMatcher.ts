...
export function normalizeIngredientString(str: string): string {
  ...
  return str
    ...
    .replace(
      /\b(fresh|dried|chopped|diced|sliced|minced|grated|crushed|ground|whole|cubed|cooked|raw|cloves of|tbsp|tsp|cups|grams|kg|ml)\b/gi,
      ''
    )
...
const SYNONYM_MAP: Record<string, string[]> = {
  ...
  potato: ['potatoes', 'baby potatoes', 'russet potato'],
};
...
export function isIngredientInPantry(
  ...
  for (const pantryItem of userPantryItems) {
    ...
    if (
      cleanIng.replace(/es$/, '') === cleanPantry ||
      cleanPantry.replace(/es$/, '') === cleanIng
    )
      return true;
    ...
    for (const [key, syns] of Object.entries(SYNONYM_MAP)) {
      if (
        (cleanIng.includes(key) || syns.some((s) => cleanIng.includes(s))) &&
 ...
 *
 * Groups results into:
 * - MAKE NOW: No required ingredients missing (missing count === 0 or match === 100%)
...
export function evaluateRecipeMatch(
  ...
  const matchPercentage =
    totalRequired > 0 ? Math.round((availableCount / totalRequired) * 100) : 100;
  ...
  const reason =
    ...
    (group === 'MAKE NOW'
      ...
      : group === 'ALMOST THERE'
      ? `You have ${availableCount} of ${totalRequired} ingredients (${matchPercentage}%). Just grab ${missingRequired
          .slice(0, 2)
          .join(' & ')}.`
      : group === 'GOOD MATCH'
      ? `Strong culinary match utilizing ${availableCount} of your kitchen items.`
      : `Exciting recipe that builds on your ingredients with a few fresh additions.`);
